        ...
        String queryStr2 = """
            ...
            GROUP BY msi.sell_date ),0) AS medicine_sales,
                ...
                COALESCE((SELECT COUNT(sc3.id)
                    ...
                    COALESCE((SELECT COUNT(ri.reg_no) FROM registration_info ri
                    WHERE ri.create_date = c.date_field AND ri.hospital_code = :hospitalCode GROUP BY ri.create_date ),0) AS new_patient,
                ...
                (COALESCE((SELECT COUNT(ri.reg_no) FROM registration_info ri
                WHERE ri.create_date = c.date_field AND ri.hospital_code = :hospitalCode GROUP BY ri.create_date ),0) +
                ...
                FROM calendar c
                LEFT JOIN registration_info ri ON ri.create_date = c.date_field AND ri.hospital_code = :hospitalCode
                LEFT JOIN service_charges sc ON sc.id = ri.service_charge_id
            WHERE c.date_field BETWEEN :fromDate AND :toDate
            GROUP BY c.date_field;
        """
