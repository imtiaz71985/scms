...
@Transactional
class GetDropDownServiceProviderTagLibActionService extends BaseService implements ActionServiceIntf  {
        ...
        public Map execute(Map result) {
            try {
                List<ServiceProvider> lstServiceProvider=ServiceProvider.findAllByIsActive(true)
                String html = buildDropDown(lstServiceProvider, result)
                result.html = html
                return result
            } catch (Exception e) {
                log.error(e.message)
         ...
         * @return - html string for select
         */
        private String buildDropDown(List<ServiceProvider> lstValues, Map dropDownAttributes) {
            // read map values
