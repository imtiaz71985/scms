...
import com.scms.SystemEntityType
...
@Transactional
class GetDropDownSystemEntityTypeTaglibActionService extends BaseService implements ActionServiceIntf {
    ...
    public Map execute(Map result) {
        try {
            List<SystemEntityType> lstSystemEntityType = SystemEntityType.list()
            String html = buildDropDown(lstSystemEntityType, result)
            result.html = html
            return result
        } catch (Exception e) {
            log.error(e.message)
     ...
     * @return - html string for select
     */
    private String buildDropDown(List<SystemEntityType> lstValues, Map dropDownAttributes) {
        // read map values
