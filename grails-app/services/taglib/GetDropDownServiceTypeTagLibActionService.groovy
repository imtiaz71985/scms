...
@Transactional
class GetDropDownServiceTypeTagLibActionService extends BaseService implements ActionServiceIntf  {
    ...
    public Map execute(Map result) {
        try {
            ...
            if(type.equals('counselor')) {
                ...
                lstServiceType = ServiceType.findAllByIsActive(Boolean.TRUE)
            }
            String html = buildDropDown(lstServiceType, result)
            result.html = html
            return result
        } catch (Exception e) {
            log.error(e.message)
     ...
     * @return - html string for select
     */
    private String buildDropDown(List<ServiceType> lstValues, Map dropDownAttributes) {
        // read map values
