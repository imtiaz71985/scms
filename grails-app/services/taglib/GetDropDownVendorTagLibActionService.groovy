...
class GetDropDownVendorTagLibActionService extends BaseService implements ActionServiceIntf  {
    ...
    public Map execute(Map result) {
        try {
            List<Vendor> vendors = Vendor.list(sort: NAME,order: ASCENDING_SORT_ORDER)
            String html = buildDropDown(vendors, result)
            result.html = html
            return result
        } catch (Exception e) {
            log.error(e.message)
     ...
     * @return - html string for select
     */
    private String buildDropDown(List<Vendor> lstValues, Map dropDownAttributes) {
        // read map values
