    ...
    public long serviceChargeIdByServiceType(long typeId){
        String queryStr = """
                    SELECT sc.id AS chargeId
                        FROM service_head_info sh
                        LEFT JOIN service_charges sc ON  sh.service_code=sc.service_code
                    WHERE sh.service_type_id=${typeId}
                        AND sh.is_active=TRUE
                        AND sc.activation_date<=CURRENT_DATE
                        AND CURRENT_DATE BETWEEN sc.activation_date AND COALESCE(sc.last_active_date,CURRENT_DATE)
                    GROUP BY sc.service_code ORDER BY sc.id desc
        """
        ...
        if(typeId>0) {
            queryStr = """
                    ...
                    sh.service_type_id AS serviceTypeId, st.name AS serviceTypeName,sc.activation_date AS activationDate,sh.is_active AS isActive
                        FROM service_head_info sh
                        INNER JOIN service_charges sc ON sh.service_code=sc.service_code
                        LEFT JOIN service_type st ON sh.service_type_id=st.id
                    WHERE sh.is_active=TRUE AND sc.activation_date<=CURRENT_DATE
    ...
    public  List<GroovyRowResult> serviceHeadInfoList(){
        String queryStr = """
                    ...
                    sh.service_type_id AS serviceTypeId,st.name AS serviceTypeName,sc.activation_date AS activationDate,sh.is_active AS isActive
                            FROM service_head_info sh
                                LEFT JOIN
                                        (SELECT MAX(COALESCE(last_active_date,CURRENT_DATE)) AS last_active_date,id,
